...
set "TOKFILE=%TEMP%\airemote-appr-token-%SFX%.txt"
set "RC=1"
...
"%PYEXE%" -c "import json,sys; json.dump({'hook_event_name':'PermissionRequest','session_id':'e2e-smoke','tool_name':'Bash','tool_input':{'command':'echo hello'},'permission_mode':'default','cwd':'.'}, open(sys.argv[1],'w',encoding='utf-8'))" "%PAYLOAD%"

echo [3/5] starting responder serve ^(auto-allows the next request^) ...
> "%RUNRESP%" echo @echo off
>> "%RUNRESP%" echo "%PYEXE%" "%APPROVER%\responder.py" serve --config "%RCFG%" ^< "%ANS%"
...
for /l %%n in (1,1,20) do (
    if not defined GOTOUT (
        "%PYEXE%" "%APPROVER%\hook.py" --config "%HCFG%" < "%PAYLOAD%" > "%OUT%" 2>nul
        if not errorlevel 1 (
            set "GOTOUT=1"
        ) else (
            ping -n 2 127.0.0.1 >nul 2>&1
...
"%PYEXE%" -c "import json,sys; d=json.load(open(sys.argv[1],encoding='utf-8')); dec=d['hookSpecificOutput']['decision']; print('    behavior :', dec.get('behavior')); sys.exit(0 if dec.get('behavior')=='allow' else 1)" "%OUT%"
if errorlevel 1 (
...
del "%HCFG%" "%RCFG%" "%ANS%" "%PAYLOAD%" "%OUT%" "%RUNRESP%" "%TOKFILE%" >nul 2>&1
if not "%RC%"=="0" (
    echo.
