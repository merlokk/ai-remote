...
| [`esp32yk-make-vectors.cmd`](#esp32yk-make-vectorscmd) | regenerate the ARKG vectors for the ESP32-S3 responder | `--check` is | no | no |
...
the board with a security key on its OTG port instead of a screen. 358 tests,
including the six suites that folder has and its sibling does not: the LED's
encoding, the state ranking above it, the three layers of CTAPHID / CBOR / CTAP2,
and **the ARKG derivation** that decides which key signs a verdict there.
...
plus one thing its twin does not need — §7's parity vectors, which live in
`approver-esp32\host_test\vectors\` and are **read from there rather than
copied**. Its *own* vectors, the ARKG ones, are committed in that folder and
regenerated by `esp32yk-make-vectors.cmd` below. There is one copy of that header in the repository on purpose; that
...
**Both boards read the first of those**, and neither has a copy. The ESP32-S3's
*own* tier-2 vectors are a different kind of fact and have their own generator —
`esp32yk-make-vectors.cmd`, below.
...
## `esp32yk-make-vectors.cmd`

The same idea for the ESP32-S3 responder, and for a sharper reason.
`make-vectors.cmd` pins the bytes a decision is signed over; this one pins **the
derivation of the key that signs it**.

That board holds no signing key of its own: it derives an ARKG public key on the
chip, registers it, and every verdict is then signed inside a security key that
reconstructs the private half from a key handle
([`approver-esp32-yubikey/key.md`](../approver-esp32-yubikey/key.md) §10.18).
Nothing checks that the two halves match. A derivation wrong by one byte registers
a key nobody can sign for, `hook.py` rejects every reply, and from the desk that
looks exactly like a device that is not answering.

```
approver-esp32-yubikey\host_test\vectors\arkg_vectors.h        every intermediate, plus two CTAP2 responses
approver-esp32-yubikey\components\arkg\arkg_selftest_vector.h  the one that ships **in the firmware**
```

The second file is what `key selftest` on the device runs the two curve operations
against — on the chip, with nothing plugged into the OTG port. It is the only part
of that design that can be checked without hardware.

```bat
scripts\esp32yk-make-vectors.cmd            REM write them
scripts\esp32yk-make-vectors.cmd --check    REM write nothing; exit 1 if stale
```
...
`tests/test_esp32yk_arkg_vectors.py` cross-checks the result against **Yubico's
own**, which is the agreement that matters.

Needs no NATS, no board and no security key. It does need the venv.
