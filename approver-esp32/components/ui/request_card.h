...
//
// **This file includes `<cstdint>`, `<cstddef>` and the navigator**, the last of
...
class RequestCard {
   public:
    // The same four the navigator bounds itself to, tied rather than repeated:
    // the two objects are stepped together by whoever owns them, and a queue
    // that could hold five while the navigator counted four would put a card on
    // screen with no navigation to come back to.
    static constexpr uint8_t kMaxPending = Navigator::kMaxPending;
