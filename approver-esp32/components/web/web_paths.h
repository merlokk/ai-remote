...
// decision-holding file in this firmware has (§10.11): the navigator, the Wi-Fi
// policy, the sync schedule, the bus link, every screen's arithmetic. The socket
...
//
//   **the same filesystem holds `config.json`.** SPIFFS carries the pages *and*
//   the Wi-Fi passphrase, the pinned handler key and the factory defaults
//   (§10.15) — so a static file server that serves whatever is asked for hands
//   the WPA password to anyone who types `/config.json`. §10.15's rule is that a
//   password is a secret from the moment it is typed: never logged, never in a
//   console dump. A URL is a bigger hole than either.
//
...
// all — it takes nothing away that comes back by itself.
