...
// interface only. This file is the first of those two, asked for by the
// repository owner and modelled on the house firmware of §10.14.4, which answers
// the same question with `users.json` and HTTP basic authentication.
//
...
// this device, its bus and its registration — off a LAN. So the rule is the
// simple one: with credentials configured, *nothing* is served without them, the
// 401 included.
//
...
// the Wi-Fi policy, the sync schedule, every screen's arithmetic. Here the reason
// is the strongest of the set: this is the one comparison standing between a
// network and a device that can be pointed at another NATS server, and a
// comparison that needs a board to exercise is one nobody exercises.
//
...
inline constexpr const char *kAuthRealm = "approver-esp32";
