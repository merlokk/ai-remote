...
// The words the page sends for the radio. They are `ui::WifiMode`'s three states
// rather than a fourth spelling of the same thing: that enum is what the screen of
// §10.8.6 cycles through and what maps onto the two config fields, and two
// mappings between the same three words and the same two booleans is one mapping
// too many.
struct ModeWord {
    const char *word;
    ui::WifiMode mode;
...
constexpr ModeWord kModes[] = {
    {"off", ui::WifiMode::kOff},
    {"client", ui::WifiMode::kClient},
    {"ap", ui::WifiMode::kAp},
};
...
bool ReadIp(const cJSON *ip, config::StaticIp *into, const config::StaticIp &keep,
    ...
    // is a bug this had before the board found it: the Wi-Fi page has no address
    // fields, so *every* Apply was quietly forgetting a static address somebody had
    // typed on the console. Found by pressing Apply and watching a
    // `192.168.1.42` turn into nothing.
    //
...
bool ReadWifi(const cJSON *wifi, config::Data *into, WriteOutcome *out) {
    ...
    for (const cJSON *item = wifi->child; item != nullptr; item = item->next) {
        ...
        if (std::strcmp(key, "mode") == 0) {
            ...
            const ui::WifiMode *found = nullptr;
            for (const ModeWord &word : kModes) {
                if (std::strcmp(word.word, item->valuestring) == 0) {
                    found = &word.mode;
                    break;
                }
            }
            ...
            // **Off leaves `wifi.mode` alone**, which is `ui::WifiView`'s rule and
            // not an omission: off is a statement about the radio, and writing the
            // mode as well would move which record the *screen* is showing under
            // an operator who only switched the radio off.
            into->wifi.active = ui::WifiModeActive(*found);
            if (into->wifi.active) {
                into->wifi.mode = ui::WifiModeIsAp(*found) ? config::WifiMode::kAp
        ...
        if (std::strcmp(key, "networks") == 0) {
            ...
            for (const cJSON *entry = item->child; entry != nullptr; entry = entry->next) {
                ...
                // and it is here because the mutation that broke the check above
                // wrote past the end of `fresh` rather than failing a test. A second
                // comparison is cheaper than trusting the first one to stay correct.
...
bool ReadNats(const cJSON *nats, config::Data *into, WriteOutcome *out) {
    ...
    for (const cJSON *item = nats->child; item != nullptr; item = item->next) {
        ...
        // console's `config set nats` makes, and for its reason: a URL that will
        // not parse is a bus the device silently never connects to, which looks
        // exactly like a bus that is down.
        if (url[0] != '\0') {
