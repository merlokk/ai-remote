...
//
// §10.16 shipped read-only and said what the write path would need before it
// existed: "a writable config over HTTP is a way to point this device at another
// NATS server, and that needs the authentication question answered first". The
// repository owner asked for the Wi-Fi and bus pages anyway, and this file is the
// answer to that sentence rather than a way round it — the whole of what a request
// may reach, in one place that needs no board to test.
//
...
//      more than a form should be able to touch (the touch calibration, the idle
//      timers, the clock), and none of it is on this list.
//   2. **A password can be written and never read back.** §10.15 keeps a
//      passphrase out of every log line and every console dump, so a JSON document
//      served over the LAN is out of the question — `GET /api/wifi` says `secured`
//      and nothing else. Which leaves the problem of a form that has to submit
//      *something*: an absent or null password means **keep the one that is
//      there**, matched by SSID, and an empty string means clear it. That is the
//      difference between "I did not retype it" and "this network is open", and it
//      is two tests.
...
//      *bus* is the exception, because dropping it costs the operator nothing they
//      are looking at. So the reconnect is its own action and the page has a
//      button for it (§10.9's lesson from `wifi check`, arrived at from the
//      other side: reach for the narrowest thing that changed).
//
// What this file is **not** is authentication. Anyone who can reach the server can
// submit the form, exactly as anyone on that LAN can already read every permission
// request off the bus (§10.3). `config.json`'s `web.write` is the switch that
// refuses the whole write path, and TLS with credentials stays the real fix.
...
// two other sections are a hundred more; 1,280 is that with room for whitespace
// and **no more than that**, because it lives in `.bss` and the heap it is taken
// from is where the Wi-Fi driver finds its transmit buffers (§10.16).
inline constexpr size_t kMaxSettingsBody = 1280;

// How a write ended. **Every refusal is its own value**, the rule §10.7 states
// for the console and §10.8.5 for the touch calibration: "that field does not
// exist" and "that address is not an address" send somebody in different
// directions, and one `false` would send them in neither.
...
// signing bytes and §10.8.5 about a calibration: a half-applied write is the one
