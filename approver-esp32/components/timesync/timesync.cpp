...
// Writes the system clock's current value into the RTC, so the next boot
// starts from a good number instead of from a drifted one. UTC, always
// (§10.8.2) — `gmtime_r`, never `localtime_r`.
esp_err_t StoreInRtc(time_t utc) {
    if (chip == nullptr || !chip->Present()) {
    ...
    rtc::DateTime value = {};
    value.year = static_cast<uint16_t>(fields.tm_year + 1900);
    value.month = static_cast<uint8_t>(fields.tm_mon + 1);
    value.day = static_cast<uint8_t>(fields.tm_mday);
    value.hour = static_cast<uint8_t>(fields.tm_hour);
    value.minute = static_cast<uint8_t>(fields.tm_min);
    value.second = static_cast<uint8_t>(fields.tm_sec);
    value.weekday = static_cast<uint8_t>(fields.tm_wday);
...
Outcome RunExchange() {
    ...
    const esp_err_t stored = StoreInRtc(after);
    if (stored != ESP_OK) {
        // The system clock is right either way, so this is a warning and not a
        // failed sync: what is lost is the head start at the next boot, which
        // the next sync will put back.
        ESP_LOGW(TAG, "clock set, but the RTC would not take it: %s", esp_err_to_name(stored));
    }
...
esp_err_t Init(rtc::Pcf85063 *clock) {
    ...
    if (time_config.sync_hours == 0) {
        ESP_LOGI(TAG, "clock sync is off; the RTC is the only time source");
