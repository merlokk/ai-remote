...
1. **Host tier — no board.** The pure logic runs under Unity on the development
   ...
   **What is under it today is the navigator, every screen's arithmetic, all
   ...
   | `components/web` | §10.16's whitelist, and the suite is almost entirely refusals — because the pages and `config.json` share one flat filesystem, so every one of them is about a WPA passphrase not leaving the device over HTTP. `/config.json`, `/registration.json` and every other `.json` refused because the extension is not on the list rather than because the name is on a blacklist; an unlisted extension refused; traversal in three spellings, including the percent-encoded one that nothing decodes; a name longer than SPIFFS can store refused **at the bound and one over it**; a buffer too small refused with the caller's buffer untouched (§10.2's rule about a refusal writing nothing); `/` being the index, with and without a query string; and the six files this site is made of all passing, `404.html` included — the server opens that one by name rather than from a URL, so nothing forces it through the whitelist and it is asserted to pass anyway. **And the reboot's confirmation**, which is a scan over the query rather than a `strstr` for one reason worth a test each: `xconfirm=reboot` and `confirm=reboots` both *contain* the word and neither confirmed anything, a path that spells it is not a query, and the comparison is case-sensitive like every other one in that file. Plus the content types, and that one is never null — that fallback is unreachable through the whitelist today, and is kept because "unreachable today" is not "unreachable". **And the write path** (§10.16), which is the only way anything outside this device changes what it does, so the suite is a list of things that must not get through: a field that is not on the whitelist refused **by name** — the touch calibration, the display, the clock, the audio, `wifi.rounds`, and `web.write` itself, which would otherwise make the read-only switch a suggestion — a mode that is not one of the three refused rather than guessed, a URL that will not parse, a string longer than its field refused rather than cut, a fifth network refused rather than dropped, a nameless one refused, an address with a leading zero and one with an octet over 255, and every one of them asserted to leave the settings **byte for byte** as they were. Then the four that are not refusals and carry the design: a password that was not retyped is kept **by name rather than by position**, an empty one clears it, a new network starts with nobody else's, and an address the form does not carry survives — that last one is a bug the board found by pressing Apply and watching a `192.168.1.42` disappear. Plus the four verbs, where a value that merely starts with one is not it. **And who may reach any of it** (§10.16's gate, `web_auth.h`): it is off until the config carries *both* a user and a password — so neither half alone locks anything, which is three assertions and the reason the readout has a word for that state; the right credential gets in; and then every way one can be wrong, because this is the comparison standing between a network and a device that can be pointed at another NATS server. No header at all — which is what a browser's *first* request looks like, every time — a wrong password, a wrong user, the colon moved (`admi:nsecret` is not `admin:secret`, which is what comparing the *encoded* form gets for free), another scheme, a scheme that merely starts like ours, nothing after the scheme, and **a credential that is right up to its last character** — the one mistake a hand-written compare walks straight into and a fuzzer never finds. Plus the tolerances that are not a wrong password and would cost somebody an afternoon: extra spaces, a tab, a trailing CRLF, and a scheme token in any case, since RFC 7235 says it is case-insensitive while the base64 after it is data. And the encoder against RFC 4648's three padding residues, because the padding is where one written by hand goes wrong; the longest credential this device can hold still encoding, which is what ties the buffer to `config::kWebUserSize` and `kWebPasswordSize`; one too long **refused with the caller's buffer untouched** *and* the gate refusing rather than admitting everybody, which is the fail-closed half of the same sentence; and a colon in the user name refused at the encoder, while one in the password works — everything after the first colon is the password. **And that the credential cannot be written from the site it locks**, which is on the write-path list above where it belongs. **And `web::Next`, which is the reconciler's whole decision and one double free**: the rule that a *held* lifetime freezes it — asserted over every combination of the other five inputs, because "the world changed underneath" must never be a reason to touch a server another task owns (§10.16 has the panic that bought it) — plus the three ways the answer turns to stop, the two that start, and the pair that must stay silent, since a tick acting on a world it already agrees with is a start on top of a running server five times a second. **And `web::ShouldRun` under it, which is four lines and two panics**: no network stack means no server in every mode, `off` stays off with a network, `on` is up on either kind of network, `auto` waits for an access point — and a radio that is merely *no longer wanted* takes the server with it while the stack is still up, which is the ordering that stopped `wifi mode off` rebooting the board |
   ...
   | `components/ui` + `components/protocol` (the limits) | §9.2, §9.7 and §10.8.3, and the fixture is §9.7's own example document. **The two traffic-light scales**, both boundaries of each, and the assertion §10.8.3 asks for by name — that they cannot drift into each other, checked at every percentage from 0 to 100 rather than at a sample. `countdown()` against `render.rs`'s own cases, a reset in the past reading `now` rather than underflowing. **Absent is absent**: a document with no `rate_limits` is ordinary, and one window without the other is read. `ts` as the only required field, junk keeping the last good document, a percentage clamped and *rounded*, and a field too long **truncated** rather than refused — the one place in this firmware where that is right, and the test says so. Then the arrival rules: a document raises the screen, the minute is checked at both sides of its boundary and across the ~49-day wrap, a stream that keeps arriving keeps the screen, and a dismissal survives ten more documents but not the silence after them. **And the screen nobody arrived at**, which is the state a board sat in overnight (§10.8.3): a visit by hand while the stream is quiet gets a minute of its own, checked at both sides of its boundary and across the wrap; it is *not* armed over a live stream, a document arriving takes it over, and leaving the screen ends it. Plus the two things the glass then says — the document going stale a minute after it lands, and the age reading in the units the console prints, `66 m` rather than `4000 s`, at every band boundary and into a buffer three bytes long |
   | `components/ui` + `components/protocol` (what it is doing) | §9.10's document and the line it becomes, and it is a suite of its own next to the limits for the same reason the two are separate subjects: one is what a session is spending, the other what it is doing, and a failure should say which half. The fixture is §9.10's own example document. **`v` is the "this is ours" test** — absent, a string, or a version from a newer publisher are three refusals, and each leaves the last good document standing; so does an `event` or a `state` this firmware has no word for, `Stop` with a capital included, because everything downstream takes an enum. `post_tool` is asserted to be `thinking` and never `idle`. Then the field work: text too long **truncated** rather than refused, and the truncation **backing off to a UTF-8 boundary** — 200 two-byte characters into a 128-byte field, with every byte of the result asserted to be a whole pair, because half a character is a placeholder box on the panel next to a perfectly good line. The headline's five shapes (tool and summary, a subagent ahead of both, a tool with nothing worth quoting, a document with no tool at all reading as its state, and **a `post_tool` whose tool is read and deliberately not drawn** — `thinking` instead, because a finished tool named on the glass is indistinguishable from a running one), that it is ASCII — the panel draws Montserrat's default subset — and that a buffer far too small comes back terminated rather than overrun. And the asymmetry the screen is built on: a `running` document goes stale at ten minutes, checked either side of the boundary, and an `idle` one never does |
   ...
   **The activity line added two, both caught.** Reading `v` and not comparing it
   fails the version test — the document a newer publisher would send is accepted
   and drawn — and cutting the summary on a byte boundary instead of a character
   boundary fails the Cyrillic one. The second is the one worth having: it is the
   only difference between this parser and `status.cpp`'s, so a tidy-up that
   "unified" them would land exactly there.
   ...
   **The reconciler added two, both caught, and it is the shortest entry here
   because the subject is four lines.** Removing the `held` check makes the
   reconciler answer `kStop` while a diagnostic owns the server, which is the
   double free of §10.16 back in the code; removing the `want == running` check
   makes it act on a world it already agrees with, which on the board is a start
   on top of a running server five times a second. Both had to be written as
   `&& false` rather than deleted — `/W4 /WX` turns an orphaned parameter into a
   build error, the trap this section records four times now — and the harness
   `os.utime`s the file it restores, without which Ninja rebuilds nothing and the
   next mutation is measured against the previous one's binary.
   ...
   **all three are in the table above now**: the navigation state machine (a
   request preempts every screen; it cannot be dismissed; what was underneath
   comes back with its state — §10.8.1), the Wi-Fi state machine (backoff
   bounded, auth failure sticky and distinct from "not found", `forget` returning
   to `NO_CREDENTIALS` — §10.9), and the limits screen's arithmetic —
   `countdown()` against `render.rs`'s own cases, the staleness threshold, and the
   §9.2 gauge scales pinned exactly as `render.rs` and `statusline.ts` pin them,
   including the assertion that the two cannot drift into each other. That last
   one was owed here for a while and is `test_limits.cpp` now; the clock face
   joined them as well.

   One shared constant is worth keeping in view because two subjects hold it: the
   clock face's two-minute traffic window is §10.8.3's staleness threshold, and
