...
// socket at a time, which is why there is no <link> in any page and why the CSS is
// injected from here.
//
// On the sibling board that rule was forced: three parallel requests against about
// 18 KB of free internal RAM was a page that hung. This board has 8 MB of PSRAM
// behind lwIP and does not need it — the shape is kept anyway, because the pages
// are the same pages and a site that only works on the roomier of the two devices
// is a site nobody can move between them.
//
// What differs here is what the front page has to say: there is no battery to
// draw a gauge for and no screen to report on, and there are two things the
// sibling has nothing to show — the security key on the OTG port, and what this
// device's one light is saying (10.17). `paint` below is where that lives.
(function () {
  "use strict";
  ...
  // The order is the device's own ranking (10.17), lowest rung first: a device
  // with no enrolment cannot register, so telling somebody to spend a one-time
  // token would cost them the token. `blocked_by` is the responder's own word for
  // whatever is left, so this page never invents a diagnosis of its own.
  function lampFor(d) {
    if (d.subscribed) return { cls: "ok", word: "ready" };
    if (!d.key_enrolled) return { cls: "bad", word: "no key enrolled" };
  ...
  function frontPage() {
    function paint(d) {
      ...
      // The ranking's own word for what the emitter is doing, so that this page
      // and the light on the desk cannot disagree about what the device is.
      text(by("light"), d.light || "-");
      text(by("key-id"), d.key_id || "-");
      text(by("received"), d.received);
      text(by("answered"), d.replied + " (" + d.allowed + " allow, " + d.denied + " deny)");
      // **Its own row, never summed into the denies.** Every verdict here costs a
      // touch, so "nobody touched the key" is the ordinary third answer, and a
      // page that called it a refusal would be reporting refusals nobody made.
      var declined = by("declined");
      declined.textContent = d.declined + (d.declined ? " (no touch, or the key said no)" : "");
      declined.className = d.declined ? "warn" : "faint";

      // The key. Two facts, and neither of them is "does it work" — that is a
      // console command, it costs two curve operations, and it asks for a touch.
      var present = by("key-present");
      present.textContent = d.key_present ? "yes" : "nothing plugged in";
      present.className = d.key_present ? "ok" : "warn";
      var enrolled = by("key-enrolled");
      enrolled.textContent = d.key_enrolled ? "yes" : "no - 'key enrol' on the console";
      enrolled.className = d.key_enrolled ? "ok" : "bad";
      ...
      // Internal RAM, which is the number that decides whether this device is
      // safe: task stacks and static buffers come out of it and cannot live in
      // PSRAM. The lowest figure ever seen is the one worth reading.
      text(by("heap"), bytes(d.heap_free) + " internal, lowest " + bytes(d.heap_low));
      // And the roomy half on its own row rather than added to the line above,
      // for the same reason the console gives it its own line: a total of eight
      // and a half megabytes would say nothing about the memory that is scarce.
      var psram = by("psram");
      if (typeof d.psram_free === "number" && d.psram_free > 0) {
        psram.textContent = bytes(d.psram_free) + " free";
        psram.className = "";
      } else {
        // Not a footnote: this build expects some, and without it every stack and
        // buffer competes for internal RAM.
        psram.textContent = "none - this build expects some";
        psram.className = "bad";
      }
