...
esp_err_t Parse(const char *json, Data *out) {
    ...
    if (cJSON_IsObject(led)) {
        ...
        CopyNumber(led, "brightness", 0, 100, &value);
        out->led.percent = static_cast<uint8_t>(value);

        value = out->led.idle_percent;
        CopyNumber(led, "idleBrightness", 0, 100, &value);
    ...
    if (cJSON_IsObject(approval)) {
        ...
        long value = out->approval.touch_timeout_seconds;
        CopyNumber(approval, "touchTimeoutSeconds", 1, 600, &value);
        out->approval.touch_timeout_seconds = static_cast<uint16_t>(value);
    }
...
esp_err_t Serialise(const Data &in, size_t *length) {
    ...
    if (led != nullptr) {
        ...
        cJSON_AddNumberToObject(led, "idleBrightness", in.led.idle_percent);
    }
...
void FillDefaults(Data *out) {
    ...
    *out = Data{};
    ...
    snprintf(out->wifi.ap_ssid, sizeof(out->wifi.ap_ssid), "approver-yubikey");
    ...
    out->approval.touch_timeout_seconds = 30;
    out->approval.deny_button = true;
}
