...
// board of the sibling folder it also included `navigator.h` — and did not use a
// symbol from it, which the compiler here noticed the moment the navigator went
// with the screens. Removing it is the only difference between the two copies;
// everything else must stay identical, because `components/protocol` is shared
// with that folder byte for byte and a struct that drifts is a document that
// stops parsing.
//
// The original note, whose point stands:
//
// **It included `<cstdint>`, `<cstddef>` and the navigator**, so all of it
