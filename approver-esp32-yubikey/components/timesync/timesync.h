...
// **No RTC argument, unlike the sibling board's.** There is no battery-backed
// clock on the YD-ESP32-S3 (§10.13), so there is nothing to seed the system time
// from at boot and nothing to write it back to after a sync. The time is whatever
// SNTP last said, and a power cut loses it.
//
// What that costs is deliberately small: **nothing in the approval path reads the
// clock.** §7's `ts` is echoed from the request rather than re-derived here
// (`signing.h` says so), so a device with the wrong time still produces signatures
// that verify. What is affected is log timestamps and `date` on the console, both
// of which say so when the clock has never been set.
esp_err_t Init();
bool Ready();
