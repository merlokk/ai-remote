...
// decoded stack from the board that found it.
//
...
// public entry points take it, and nothing that already holds it calls something
...
// `portMAX_DELAY` rather than a timeout-and-skip, which is the right answer for a
// bus that a dropped frame can survive and the wrong one for a pointer that must
// not be freed twice.
class Held {
  public:
...
// re-entered. The alternative is half a kilobyte on the server's own stack, and
// §10.14.1's rule is that nothing large goes on one.
...
// as invalid JSON that the page reports as such. The fields below come to a little
// over 600 bytes with the longest SSID, key id and blocker text this device holds.
char g_json[1024];
...
// Where the approval counters, the key's state and the light's word come from,
// and why it is a hook rather than a call: `web_server.h` has the argument.
...
// `ESP_LOG` line from the responder or the USB host's daemon. Two things follow,
...
// anybody else's go where they were going in the first place — the console on the
// CH343P bridge — which is also why nothing is lost from the log while a page is
// served.
struct Sink {
    httpd_req_t *request;
    TaskHandle_t owner;
...
// The heap numbers, in one place so the three call sites cannot drift. **Internal
// RAM**, which `web_server.h` argues: on a board with 8 MB of PSRAM the total is
// not the scarce number and reporting it would be reporting nothing.
void Sample() {
...
// `fido.json` that it is there. The difference is counted and logged on this
...
// lines about this device, its bus, its registration and the key on its port —
// off a network. So with a credential configured, nothing is served without one,
// and with none configured nothing changes at all. `web_auth.h` has that rule and
// the three others; what is here is the socket.
esp_err_t Unauthorised(httpd_req_t *request) {
    ...
    // lockout. On this board that would be worse than on the sibling: there is no
    // screen to read an address off and the console is the only other way in.
...
// cached at start, so `web login` and `config reload` take effect without the
...
esp_err_t StatusApi(httpd_req_t *request) {
    ...
    // request with no credential must not reach a counter or a buffer. With
    // nothing configured this is a comparison against two empty strings and lets
    // everybody through, which is what it did before authentication existed.
    if (!Allowed(request)) {
    ...
    // **No password, no `ikm`, and no key handle.** §10.15 keeps a passphrase out
    // of every log line and every console dump; a URL is a wider audience than
    // either, and §10.18's enrolment is on the same filesystem as these pages.
    // What is here is state — and the `key_id` is a public name rather than a
    // secret (§10.2), as is the derived public key it stands for.
    //
    // The three groups are what the front page is *for*: whether this device could
    // answer a request at all (the bus, the registration, the subscription, the
    // key), what it has answered (§7's counters, with `declined` kept apart from
    // `denied` because §10.10 rule 2 says they are different outcomes), and
    // whether it is healthy (heap, PSRAM, uptime, filesystem).
    std::snprintf(g_json, sizeof g_json,
                  "{\"firmware\":\"%s\",\"idf\":\"%s\",\"uptime_s\":%" PRIu32
                  ",\"heap_free\":%" PRIu32 ",\"heap_low\":%" PRIu32 ",\"psram_free\":%" PRIu32
                  ",\"spiffs_used\":%u"
                  ",\"spiffs_total\":%u,\"wifi\":\"%s\",\"ssid\":\"%s\",\"rssi\":%d"
                  ",\"ip\":\"%u.%u.%u.%u\""
                  ",\"bus\":\"%s\",\"bus_up\":%s,\"registered\":%s,\"key_id\":\"%s\""
                  ",\"ready\":%s,\"subscribed\":%s,\"blocked_by\":\"%s\""
                  ",\"key_present\":%s,\"key_enrolled\":%s,\"light\":\"%s\""
                  ",\"received\":%" PRIu32 ",\"allowed\":%" PRIu32 ",\"denied\":%" PRIu32
                  ",\"declined\":%" PRIu32 ",\"replied\":%" PRIu32
                  ",\"web_files\":%" PRIu32 ",\"web_api\":%" PRIu32 "}",
                  app != nullptr ? app->version : "?", app != nullptr ? app->idf_ver : "?",
                  static_cast<uint32_t>(esp_timer_get_time() / 1000000), g_status.free_now,
                  g_status.low_water,
                  static_cast<uint32_t>(heap_caps_get_free_size(MALLOC_CAP_SPIRAM)),
                  static_cast<unsigned>(used), static_cast<unsigned>(total),
                  wifimgr::Name(wifi.state), wifi.radio.ssid,
                  ...
                  static_cast<unsigned>((wifi.radio.ip >> 24) & 0xFF), nats::Name(bus.state),
                  ...
                  approvals.blocked_by, approvals.key_present ? "true" : "false",
                  approvals.key_enrolled ? "true" : "false", approvals.light,
                  approvals.received, approvals.allowed, approvals.denied, approvals.declined,
                  approvals.replied, g_status.requests, g_status.api);
...
esp_err_t DevStatusApi(httpd_req_t *request) {
    // **The gate, and it is the first statement in every handler** (§10.16).
    if (!Allowed(request)) {
        return Unauthorised(request);
    }
    ...
    // it does — formatting every console readout this device has on this task's
    // 4 KB (§10.14.1: the low-water mark is the number).
    //
    // **`key` is one of those readouts and it does not touch the key**, which is
    // the reason this endpoint is safe to serve to a LAN at all: the section
    // prints what `fido.json` and the last conversation said, and `key info` — the
    // one that talks to the authenticator — is a command an operator types.
...
esp_err_t SettingsGet(httpd_req_t *request) {
    ...
    int written = std::snprintf(
        ...
        // The three words, through the one mapping this board has
        // (`web_settings.h` says why it is not `ui`'s).
        WifiWordName(wifi.active, wifi.mode == config::WifiMode::kAp), wifi.ap_ssid,
        wifi.ap_password[0] != '\0' ? "true" : "false",
...
// two**, which on this task is allowed: somebody asked for it, and a request is a
// thing that takes as long as it takes. `wifimgr::Scan` says the same rule from
// the other side — only when somebody is looking at a list, never on a timer.
esp_err_t WifiScanApi(httpd_req_t *request) {
    ...
    if (err != ESP_OK) {
        ...
        // "nothing on the air" and "the radio refused" send somebody in different
        // directions.
    ...
    for (size_t i = 0; i < found && written > 0; ++i) {
        if (g_scan[i].ssid[0] == '\0') {
            continue;  // hidden: a row with no name is nothing to pick
        }
        bool already = false;
...
bool ReadBody(httpd_req_t *request, size_t *length) {
    *length = 0;
    ...
    while (taken < request->content_len) {
        const int got =
            httpd_req_recv(request, g_body + taken, request->content_len - taken);
        if (got <= 0) {
...
esp_err_t ActionApi(httpd_req_t *request) {
    ...
    switch (action) {
        ...
        case Action::kReload:
            ...
            //
            // On this board the hook also re-reads `web.mode` (`main`'s
            // `SettingsChanged`), so a reload can take the *server* down too when
            // the file says `off`. Same rule, one step further.
            err = config::Reload();
            break;
...
// --- The one thing this server can do to the device (§10.16) ---------------
//
// `POST /api/reboot?confirm=reboot`. Three rules are what make it defensible
...
//     the trust boundary at the router;
...
//     go. The enrolment and the registration are files and survive it, which is
//     what makes this cheap on this board rather than frightening.
esp_err_t RebootApi(httpd_req_t *request) {
    // **The gate, and it is the first statement in every handler** (§10.16) — and
    // this is the handler where being one statement lower would have been visible.
    if (!Allowed(request)) {
        return Unauthorised(request);
    }
...
esp_err_t File(httpd_req_t *request) {
    ...
    httpd_resp_set_type(request, ContentType(name));
    // **A phone should fetch these once.** A page's script does not change between
    // two visits to the same firmware, and a day is long enough to make a second
    // visit free and short enough that a reflash is not a support call. The pages
    // that carry *state* are the API answers, which say nothing about caching and
    // are therefore not cached.
    httpd_resp_set_hdr(request, "Cache-Control", "max-age=86400");

    // **Streamed, never read whole**, and the rule outlives the file that
    // motivated it on the sibling board: there is no 460 KB splash image on this
    // filesystem — the largest thing here is `app.js` — but a handler that read a
    // file into RAM would be one dropped-in asset away from the allocator failing
    // under a device that also holds the Wi-Fi stack and the USB host.
    for (;;) {
...
esp_err_t StartLocked() {
    ...
    // (Invalid mbox)` and reboots the device. The sibling board found it by doing
    // exactly that on a freshly flashed board, which is the one state where the
    // radio is off by default — and the shipped `config.json` here is the same.
