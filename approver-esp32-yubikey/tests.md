...
| **walking away after the tap** | a finger, and the patience not to use it | tap BOOT and then do nothing: the deny must go **unsigned and unpublished**, counted as `nothing` rather than as a deny (§10.10 rule 2). The signed half works now; this is the half that has to keep not working |
