...
| **A clock** | no RTC (no I²C bus for one) and no SNTP. `components/timesync`, `components/timezone`, the `date` command, `config zones` and the `time` section of `config.json` are deleted: §7's `ts` is echoed from the request, nothing else reads a wall-clock time, and there is nowhere to show one (§10.13) |
