...
| Console on UART0 (§10.7) | **runs** | all 17 of ours answer — 16 before §10.16 added `web` — and so does ESP-IDF's `help`, which lists all 17 because every one of them carries a description (a null one is silently left out of that listing) |
...
| A touched success through the new read path | **runs** | twice: a request off `approvals.*` came back signed and `hook.verify_reply` called it `TRUSTED`, and a `key test` was approved on the console (`gate 1 approved`). Both after the rewrite |
| Unplug mid-read | **runs** | the second half of the transfer fix, and the key has now been pulled out of the socket while the board was inside a read. The blocked exchange came back as `the key was unplugged`, the light went `ready → no-fido-key`, and **all six teardown checks passed**: nothing was left in flight, and `usb_host_interface_release` and `usb_host_device_close` both returned `ESP_OK` — which is the assertion that matters, because neither can while a pipe still has URBs on it. `cable 2 attached, 1 detached, 2 claimed`; re-enumeration and a working exchange 4 s later; `0 transfer` errors; heap **−36 bytes** across the cycle with `lowest ever` unmoved |
| — the library's own noise on that path | **fixed, and it was four lines** | halting an endpoint on a device that has already gone earns `EP command error: ESP_ERR_INVALID_STATE` from inside `usb_host.c`, which logs it at `ESP_LOGE` and cannot be talked out of it. On a gone device the library has already cancelled the transfers, so the calls achieved nothing: `CloseDevice` skips them now and `Reclaim` refuses the same work the moment `want_close` is set. Four red lines per unplug → one → **zero**, measured across three runs |
