...
void test_the_enrolment_is_not_a_page_either(void) {
    // **This board's own case, and it is sharper than the one above** (§10.18).
    // `fido.json` holds the `ikm` and the key handle the ARKG derivation needs;
    // §10.18's property that "the key alone cannot approve anything either" is
    // exactly the claim that those bytes are not public, and a `/fido.json`
    // served to a LAN would retire it.
    //
    // It is refused by the same rule as everything above rather than by a name of
    // its own, which is the argument for a whitelist stated as a test: nobody had
    // to remember this file when it was added to that filesystem.
    const char *never[] = {"/fido.json", "/fido.json.new", "/FIDO.JSON"};
    for (const char *uri : never) {
        char name[64] = {};
        TEST_ASSERT_FALSE_MESSAGE(Name(uri, name), uri);
    }
...
void test_an_extension_nobody_listed_is_refused(void) {
    // `splash.bin` and `alert.wav` are the sibling board's files and are not on
    // this filesystem at all — they stay in the list because what is being tested
    // is the *extension* rule, and a name that is not there is refused for one
    // more reason rather than one fewer.
...
void RegisterWebPathTests(void) {
    ...
    RUN_TEST(test_the_enrolment_is_not_a_page_either);
