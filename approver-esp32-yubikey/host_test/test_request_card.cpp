...
void test_the_queue_is_bounded_and_the_bound_is_four(void) {
    // **Four, and on this board it is a number rather than a reference to one.**
    // The sibling firmware ties this to `Navigator::kMaxPending` because a queue
    // that could hold five while the navigation counted four would put a card on
    // screen with nothing to come back to. There is no navigation here, so the
    // number stands on its own argument (`request_card.h`): four requests waiting
    // at once is already a session that has got ahead of its operator.
    TEST_ASSERT_EQUAL_UINT8(4, RequestCard::kMaxPending);
...
void RegisterRequestCardTests(void) {
    ...
    RUN_TEST(test_the_queue_is_bounded_and_the_bound_is_four);
