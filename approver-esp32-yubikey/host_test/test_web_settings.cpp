...
config::Data Device() {
    ...
    std::snprintf(data.wifi.ap_ssid, config::kSsidSize, "%s", "approver-yubikey");
...
void test_web_the_mode_is_three_words_over_two_fields(void) {
    // **The sibling board maps these through `ui::WifiMode` and this one cannot**:
    // that enum exists because a Wi-Fi *screen* cycles through it, and there is no
    // screen here. So `web_settings.h` owns the mapping, which is why it is worth
    // a test of its own rather than being covered by the screen's.
    //
    // `off` deliberately leaves `wifi.mode` alone: off is a statement about the
    // radio, not about which record the device comes back on.
...
void test_web_the_three_words_map_both_ways(void) {
    // The mapping itself, exercised directly rather than only through a document —
    // it is `web_settings.h`'s own now (there is no `ui::WifiMode` here), so it is
    // this file that has to say it round-trips.
    web::WifiWord word = web::WifiWord::kAp;
    TEST_ASSERT_TRUE(web::WifiWordFrom("off", &word));
    TEST_ASSERT_EQUAL_INT(static_cast<int>(web::WifiWord::kOff), static_cast<int>(word));
    TEST_ASSERT_TRUE(web::WifiWordFrom("client", &word));
    TEST_ASSERT_EQUAL_INT(static_cast<int>(web::WifiWord::kClient), static_cast<int>(word));
    TEST_ASSERT_TRUE(web::WifiWordFrom("ap", &word));
    TEST_ASSERT_EQUAL_INT(static_cast<int>(web::WifiWord::kAp), static_cast<int>(word));

    // Refused, never guessed — including the spellings that look like an answer.
    TEST_ASSERT_FALSE(web::WifiWordFrom("yes", &word));
    TEST_ASSERT_FALSE(web::WifiWordFrom("on", &word));
    TEST_ASSERT_FALSE(web::WifiWordFrom("Off", &word));
    TEST_ASSERT_FALSE(web::WifiWordFrom("", &word));
    TEST_ASSERT_FALSE(web::WifiWordFrom(nullptr, &word));

    // And back, which is what the GET hands a form to fill itself from. `active`
    // false is `off` whatever the mode says, which is the pair `config.h` argues.
    TEST_ASSERT_EQUAL_STRING("off", web::WifiWordName(false, false));
    TEST_ASSERT_EQUAL_STRING("off", web::WifiWordName(false, true));
    TEST_ASSERT_EQUAL_STRING("client", web::WifiWordName(true, false));
    TEST_ASSERT_EQUAL_STRING("ap", web::WifiWordName(true, true));
}
...
void test_web_a_field_that_is_not_on_the_list_is_refused(void) {
    ...
    AssertRefused("{\"internet\":{\"check\":false}}", WriteResult::kUnknownField);
}

void test_web_the_gate_cannot_be_reached_from_a_network(void) {
    // **This board's own §10.10 test, and the reason the whitelist is more than
    // tidiness here.** `approval` decides *when a verdict may be asked for*: the
    // timeout a request waits for a fingertip, and whether BOOT can refuse one.
    //
    // A network that could set the timeout to zero could make every request expire
    // unanswered — silence, which is §10.10's safe outcome, but silence an
    // operator did not choose. A network that could switch `denyButton` off could
    // take away the one refusal that costs no touch. Neither is on the list.
    AssertRefused("{\"approval\":{\"touchTimeoutSeconds\":0}}", WriteResult::kUnknownField);
    AssertRefused("{\"approval\":{\"denyButton\":false}}", WriteResult::kUnknownField);
    AssertRefused("{\"approval\":{}}", WriteResult::kUnknownField);
}

void test_web_what_the_light_says_cannot_be_reached_from_a_network(void) {
    // §10.17 compiles the palette in rather than making it settable, because "an
    // operator who can recolour `denied` can build a device that lies about what it
    // did". The brightness *is* a file field — and a brightness of zero written
    // over HTTP is a device that answers requests with no light at all, on a board
    // whose entire user interface is that light. The console is where it is turned
    // down.
    AssertRefused("{\"led\":{\"brightness\":0}}", WriteResult::kUnknownField);
    AssertRefused("{\"led\":{\"idleBrightness\":0}}", WriteResult::kUnknownField);
}
...
void RegisterWebSettingsTests(void) {
    RUN_TEST(test_web_the_mode_is_three_words_over_two_fields);
    RUN_TEST(test_web_the_three_words_map_both_ways);
    ...
    RUN_TEST(test_web_the_gate_cannot_be_reached_from_a_network);
    RUN_TEST(test_web_what_the_light_says_cannot_be_reached_from_a_network);
