...
void test_config_the_shipped_defaults_parse(void) {
    ...
    TEST_ASSERT_TRUE(config::Get().led.percent <= 100);
    TEST_ASSERT_TRUE(config::Get().led.idle_percent <= 100);
}
...
void test_config_the_gate_round_trips(void) {
    FreshWithDefaults();
    TEST_ASSERT_EQUAL_INT(ESP_OK, config::Init());
    ...
    TEST_ASSERT_EQUAL_UINT8(3, config::Get().led.idle_percent);
}
...
void test_config_a_partial_file_keeps_the_defaults_for_the_rest(void) {
    FreshWithDefaults();
    fake::PutFile("config.json", R"({"led":{"idleBrightness":12}})");

    TEST_ASSERT_EQUAL_INT(ESP_OK, config::Init());
    config::Data expected = {};
    config::FillDefaults(&expected);

    TEST_ASSERT_EQUAL_UINT8(12, config::Get().led.idle_percent);
...
void test_config_a_truncated_file_is_restored(void) {
    FreshWithDefaults();
    fake::PutFile("config.json", R"({"led":{"brightness":45)");
...
void test_config_valid_json_that_is_not_an_object_is_restored(void) {
    ...
    const char *const not_configs[] = {"[]", "42", "\"hello\"", "[{\"led\":{\"brightness\":9}}]"};
    ...
    for (const char *json : not_configs) {
        ...
        char back[config::kMaxFileSize] = {};
        TEST_ASSERT_TRUE(fake::GetFile("config.json", back, sizeof(back)));
        TEST_ASSERT_NOT_NULL_MESSAGE(std::strstr(back, "brightness"), json);
...
void test_config_a_negative_number_is_clamped_and_not_wrapped(void) {
    ...
    // -5 reaches `uint8_t` as 251 — a brightness that is wrong in a way that
    // looks deliberate — and a negative touch timeout reaches `uint16_t` as a
    // number that would let a request wait eighteen hours.
    FreshWithDefaults();
    fake::PutFile("config.json",
                  R"({"led":{"brightness":-5,"idleBrightness":900},
                      "approval":{"touchTimeoutSeconds":-20}})");

    TEST_ASSERT_EQUAL_INT(ESP_OK, config::Init());
    TEST_ASSERT_EQUAL_UINT8(0, config::Get().led.percent);
    TEST_ASSERT_EQUAL_UINT8(100, config::Get().led.idle_percent);
    // The gate's own floor is 1 rather than 0: a timeout of zero would be a
    // request that expires before anybody can be asked, which is not a setting
    // anybody means.
    TEST_ASSERT_EQUAL_UINT16(1, config::Get().approval.touch_timeout_seconds);
}
...
void test_config_a_restore_does_not_touch_the_registration(void) {
    ...
    FreshWithDefaults();
    fake::PutFile("registration.json",
                  R"({"key_id":"approver-esp32-yubikey","server_key":"AAAA"})");
...
void test_config_a_saved_setting_round_trips(void) {
    // §10.15's worked example. On the sibling board the worked setting is the
    // codec's volume; here it is the LED's brightness, which is the same shape
    // and the only setting on this device with a knob behind it.
    FreshWithDefaults();
    fake::PutFile("config.json", kGoodJson);
    TEST_ASSERT_EQUAL_INT(ESP_OK, config::Init());

    config::Get().led.percent = 33;
    config::Get().led.idle_percent = 21;
    TEST_ASSERT_EQUAL_INT(ESP_OK, config::Save());

    config::Get().led.percent = 99;  // scribble over it in memory
    TEST_ASSERT_EQUAL_INT(ESP_OK, config::Reload());
    TEST_ASSERT_EQUAL_UINT8(33, config::Get().led.percent);
    TEST_ASSERT_EQUAL_UINT8(21, config::Get().led.idle_percent);
}
...
void test_config_unknown_fields_are_lost_on_the_next_write(void) {
    ...
    // report.
    FreshWithDefaults();
    fake::PutFile("config.json", R"({"led":{"brightness":45},"future":{"thing":true}})");
    TEST_ASSERT_EQUAL_INT(ESP_OK, config::Init());
    ...
    TEST_ASSERT_NOT_NULL(std::strstr(back, "brightness"));
}
...
void test_config_a_leftover_temp_file_is_dropped(void) {
    ...
    FreshWithDefaults();
    fake::PutFile("config.json", kGoodJson);
    fake::PutFile("config.json.new", R"({"led":{"brightness":7}})");

    TEST_ASSERT_EQUAL_INT(ESP_OK, config::Init());
    TEST_ASSERT_FALSE(fake::FileExists("config.json.new"));
    // The good config won, not the leftover.
    TEST_ASSERT_EQUAL_UINT8(55, config::Get().led.percent);
}
...
void test_config_a_write_interrupted_in_the_window_is_finished(void) {
    ...
    FreshWithDefaults();
    fake::PutFile("config.json.new", R"({"led":{"brightness":7,"idleBrightness":11}})");
    ...
    TEST_ASSERT_TRUE(fake::FileExists("config.json"));
    TEST_ASSERT_FALSE(fake::FileExists("config.json.new"));
    TEST_ASSERT_EQUAL_UINT8(7, config::Get().led.percent);
    TEST_ASSERT_EQUAL_UINT8(11, config::Get().led.idle_percent);
}
