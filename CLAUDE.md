...
**And it echoes the outcome.** Neither the keystroke nor the touch tells the operator what actually left the machine, so `print_decision` (the `on_signed=` callback of `make_approval_handler`, third seam) prints the `behavior` and `sha256:<16 hex>` of the signature bytes (`signature_fingerprint`, `FINGERPRINT_CHARS`) right after the key signs:
