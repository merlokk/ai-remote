...
| `approver-esp32/` | a fourth client, **planned**: the responder as firmware — ESP32-C6-Touch-AMOLED-2.16 on ESP-IDF, one press on a screen on the desk. The board boots, drives its hardware and tests on the host; the responder itself is not started | [`approver-esp32/CLAUDE.md`](approver-esp32/CLAUDE.md) — **§10** the firmware; [`working-with-code.md`](approver-esp32/working-with-code.md) — where ESP-IDF is installed on this machine and how to get a shell that has `idf.py` |
