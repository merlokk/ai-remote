...
- **List of libraries in use** (the only approved ones):
  ...
  - **C++ / ESP-IDF (`approver-esp32/` only, §10) — approved; the library layer is written, the protocol is not.** The firmware is **C++**, with C only where a C ABI is forced, and **no dynamic memory** beyond one named exception (`std::string`) — everything is allocated statically and lives for the life of the device (§10.14.1); it is built library layer first, logic second (§10.14.2). That first layer now runs on the board — the leased I²C bus and the four chips on it, the panel, the settings file, the buttons, a console — under a host test suite that needs no board (§10.11); nothing of §6/§7 is started, which is why the map in §2 still calls this client planned. The dependency set is signed off: **ESP-IDF v5.5.3** — the version Waveshare's examples for this board are written against, and a pin rather than a requirement now that the version-sensitive half (panel, touch, LVGL) resolves and builds on the **v6.0.2** installed here; §10.12 has what that cost — and its in-tree components (`esp_wifi`/`lwip`, `mbedtls`, `nvs_flash`, `esp_hmac`/`efuse`, `esp_console`, `esp_lcd`, `esp_driver_i2s`), **cJSON** — in-tree on v5.5.x and a managed component (`espressif/cjson`) on the v6 installed here, which is delivery rather than a new library, **LVGL v9 + `espressif/esp_lvgl_port`**, the **CO5300** panel and **CST9220** touch drivers — which resolve on the registry to `lvgl/lvgl` 9.4.0, `espressif/esp_lvgl_port` 2.9.0, `espressif/esp_lcd_sh8601` 2.0.1 and `waveshare/esp_lcd_touch_cst9217` 1.0.4, two of them not the names this list guessed and each difference argued in §10.4 — and **libsodium** for Ed25519 — mbedTLS has no EdDSA and §6's server key is Ed25519 by fixed protocol. The bus client is **`debsahu/espidf-nats`** (registry, `^1.4.0`, MIT, header-only C++): it covers the §10.5 subset, supports the queue group §6 needs and the reply-to subject §7 answers into, and brings the TLS §10.3 will want. Each choice is argued in [`approver-esp32/CLAUDE.md`](approver-esp32/CLAUDE.md) **§10.4**; versions are declared in `main/idf_component.yml` and locked in `dependencies.lock` (committed), the way `uv.lock` and `Cargo.lock` pin the rest. On the host side only, `lvgl-mcp-server` renders the screens without a board (**§10.12.1**) and **`ffmpeg`** converts the boot and alert sounds to the uncompressed WAV the firmware plays (**§10.8.1**) — both are tools on this machine, linked into nothing. The host tests (**§10.11**) and the boot splash (**§10.8**) add no dependency either, deliberately: the tests are built by the MSVC already installed for the LVGL preview, against the **Unity** that ships inside ESP-IDF, and the splash is rasterised by `System.Drawing` from Windows PowerShell 5.1 — which is in the box on this platform, unlike every Python route to a font engine. None of this reaches the Python, Rust or Node halves.
...
| `approver-esp32/` | a fourth client, **planned**: the responder as firmware — ESP32-C6-Touch-AMOLED-2.16 on ESP-IDF, one press on a screen on the desk. The board boots, drives its hardware and tests on the host; the responder itself is not started | [`approver-esp32/CLAUDE.md`](approver-esp32/CLAUDE.md) — **§10** the firmware; [`working-with-code.md`](approver-esp32/working-with-code.md) — where ESP-IDF is installed on this machine and how to get a shell that has `idf.py`; [`commands.md`](approver-esp32/commands.md) — every console command the device answers |
