...
- `approver-web/` — a third responder, as a web page (Next.js + Chakra UI + React Hook Form + Zod, Node toolchain — the only non-Python part of the repo). Same subjects, same allowlist, same signing bytes as §6/§7: the Next server holds the NATS connection and the browser drives it over SSE + `POST /api/decision`, so nothing in `approver/` or `nats/` changed for it. **The loop is closed:** a `Register` panel takes a one-time §6 token, generates a P-256 pair (`src/lib/keys.ts`, encodings matched to `lib/crypto.py` — 32-byte scalar / 33-byte compressed point / DER signature), registers the public half and persists **only on `ok:true`**; from then on every allow/deny is signed and `hook.verify_reply` accepts it. Unregistered, it answers with `sig: ""` — rejected, so Claude Code falls back to its own prompt, never a silent allow. The private key sits in the app's git-ignored `config.json`, exactly as `responder.py` does; moving custody into the browser (non-extractable WebCrypto) is the one open item. Its own `approver-web/CLAUDE.md` holds the architecture, the transport contract, the canonical-JSON parity rule (Python's `ensure_ascii=True` vs `JSON.stringify`), the visual direction and the key-custody discussion. Runs one `approvals.*` subscriber in the `approvers` queue group, so it competes with the Python responders — run one at a time.
