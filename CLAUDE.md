...
- `approver-web/` — a third responder, as a web page (Next.js + Chakra UI + React Hook Form + Zod, Node toolchain — the only non-Python part of the repo). Same subjects, same allowlist, same signing bytes as §6/§7: the Next server holds the NATS connection and the browser drives it over SSE + `POST /api/decision`, so nothing in `approver/` or `nats/` changed for it. **The loop is closed, with the key in the browser:** a `Register` panel takes a one-time §6 token, generates a **non-extractable** P-256 WebCrypto pair in the browser, registers the public half (compressed point, `key_type=p256`) and keeps the `CryptoKey` in IndexedDB, so it survives closing the browser and **no private key exists on disk anywhere** — the Next server relays and verifies but cannot forge a decision. Registration verifies the handler's §6 server-key signature the same way the Python approvers do (`registrationReplySigningBytes` + `verifyEd25519`, a port of the same contract) and pins `server_key` in `config.json`. Each allow/deny is signed in the browser and `hook.verify_reply` accepts it unchanged. Without a key the page offers no decision buttons at all: no reply, so Claude Code falls back to its own prompt, never a silent allow. Encodings that bite (WebCrypto gives uncompressed points and raw `r‖s`, `lib/crypto.py` wants compressed and DER) are handled in `src/lib/browser-key.ts`. Its own `approver-web/CLAUDE.md` holds the architecture, the transport contract, the canonical-JSON parity rule (Python's `ensure_ascii=True` vs `JSON.stringify`), the visual direction and the key-custody discussion. Runs one `approvals.*` subscriber in the `approvers` queue group, so it competes with the Python responders — run one at a time.
